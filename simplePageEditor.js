...
$(function(){
	...
	var update="本页面产品信息仅供参考，由于报名位置实时变动，最终价格及可报位置数量以支付时为准。此页面中产品信息最后更新时间："+nowDate.getFullYear()+"年"+(nowDate.getMonth()+1)+"月"+(nowDate.getDay()+1)+"日 "+(ampm<12?"上午":"下午")+" "+(ampm<10?"0"+ampm:(ampm>12?"0"+(ampm-12):ampm))+":"+(min<10?"0"+min:min);
	...
	function edit(){
		$('.draggableObject','#static').draggable({
			...
			var _Object={
				...
				cl:_this.css('color').colorHex().toUpperCase(),
				w:_this.css('width'),
				h:_this.css('height'),
				t:_this.css('top'),
				l:_this.css('left')
			...
			if(_Object.ff&&isLine&&isDiv){
				attrHTML+='<li>字体 : <select class="classValues" types="ff">';
			...
			if(_Object.fw&&isLine&&isDiv){
				attrHTML+='<li>字体粗细 : <select class="classValues" types="fw">';
			...
			if(_Object.cl&&isLine&&isDiv){
				attrHTML+='<li>字体颜色 : <input class="num classValues" type="text" value="'+_Object.cl+'" types="cl"/>&nbsp;<div id="colorDiv" class="colorDiv" style="	background-color:'+_Object.cl+'"></div></li>';
			}
			if(isLine){
				attrHTML+='<li><button id="attrSave">保存属性</button></li>';
			}
			$('#attr').html('').append(attrHTML);
			$('#attrSave',"#pannel").click(function(){
	...
	function childEdit(){
		$('.draggableChild','#static').draggable({
			...
			var attrHTML='<li>ID : #'+_Object._id+'</li>';
			attrHTML+='<li>对象类型 : '+_Object.type+'</li>';
			attrHTML+='<li>类名 : '+_Object.className+'</li>';
			if(_Object.ff){
			...
			attrHTML+='<li><button id="childAttrSave">保存属性</button></li>';
			$('#attr').html('').append(attrHTML);
			$('#childAttrSave',"#pannel").click(function(){
	...
	function excuteCreat(){
		...
		var dataBoardCount=_static.find('.line').length;
